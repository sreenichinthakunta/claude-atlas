#!/usr/bin/env python3
"""Local server for Atlas: editable memories + a periodically fresh dashboard.

A file:// page cannot write to disk, so editing memory files needs a server.
It binds to 127.0.0.1 only and requires a per-run token on every API call, so
nothing else on your network can reach it.

This has no telemetry feature. An earlier version tailed the active
transcript to show live token counts; it was removed. What's left is
narrower and more honest: the ability to edit files Atlas already shows you,
plus a background rescan so new sessions and edits appear without a manual
rebuild.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Atlas local edit server.")
    ...
    ap.add_argument("--refresh", type=int, default=180,
                    help="seconds between dashboard rescans (0 disables)")
    args = ap.parse_args()
    ...
    print(f"Atlas server at {url}")
    print("  127.0.0.1 only · token required · Ctrl-C to stop")
    print(f"  dashboard rescans every {args.refresh}s" if args.refresh else "  rescan disabled")
    print("  unlocks: editing memory files in place")
    if not args.no_open:
        webbrowser.open(url)
    try:
        srv.serve_forever()
